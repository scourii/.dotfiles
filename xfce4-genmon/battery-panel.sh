...
if acpi -a | grep -i "off-line" &> /dev/null; then   # if AC adapter is offline
  if [ "${BATTERY}" -lt ${LOW_BATTERY} ]; then       # if battery is less than $LOW_BATTERY
    ...
    INFO+="<span weight='Regular' fgcolor='White'>"  #text appears green if the charge is above 75
  else
    INFO+="<span weight='Regular' fgcolor='White'>" # make the text white for normal states
  fi
else # if the battery is getting charged
  INFO+="<span weight='Regular' fgcolor='White'>" # text is green while getting charged
fi
